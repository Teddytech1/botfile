...
async function connectCommand(sock, chatId, senderId, message, rawMessage, prefix) {
    ...
    if (!input) {
        return await sock.sendMessage(chatId, {
            text: `🚀 *Quick Bot Deployment*\n\n` +
                  ...
                  `*Example:*\n${prefix}connect TEDDY-XMD:~eyJub2lzZUt...\n\n` +
    ...
    try {
        if (input.toLowerCase() === 'list') {
            ...
            let list = `🤖 Your Deployments (${userDeployments.length}/100)\n\n`;
            ...
            if (!input.startsWith('TEDDY-XMD:~')) {
                await sock.sendMessage(chatId, { 
                    text: '❌ Session must start with TEDDY-XMD:~' 
                }, { quoted: message });
                return;
            }
